...
export const Route = createFileRoute("/salario")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Modelo de regressão linear múltipla treinado sobre o dataset de vagas para estimar a faixa salarial de um perfil." },
    ],
  }),
...
function SalarioPage() {
  ...
  const scatterMax = Math.max(
    ...model.predictions.map((p) => Math.max(p.actual, p.predicted)),
  );
      ...
      <main className="flex-1 mx-auto w-full max-w-7xl px-6 py-12">
        <div className="mb-10 max-w-2xl">
          ...
          <p className="mt-3 text-muted-foreground">
            Modelo de <strong className="text-foreground">regressão linear múltipla</strong> treinado <em>from scratch</em>
            {" "}via equações normais <span className="font-mono text-accent">β = (XᵀX)⁻¹Xᵀy</span>, usando senioridade,
            área, modelo de trabalho e habilidades como variáveis preditoras.
          </p>
        </div>
        ...
        <div className="grid sm:grid-cols-4 gap-4 mb-10">
          {[
            { label: "R² (teste)", value: model.metrics.r2.toFixed(3), hint: "Variância explicada" },
            { label: "MAE", value: `R$ ${fmt(model.metrics.mae)}`, hint: "Erro absoluto médio" },
            { label: "RMSE", value: `R$ ${fmt(model.metrics.rmse)}`, hint: "Raiz do erro quadrático" },
            { label: "Amostras", value: `${model.metrics.nTrain} / ${model.metrics.nTest}`, hint: "Treino / Teste" },
          ].map((m) => (
        ...
        <div className="grid lg:grid-cols-[420px_1fr] gap-8">
          ...
          <Card className="p-6 bg-card border-border/50 h-fit lg:sticky lg:top-24">
            <h2 className="font-semibold flex items-center gap-2"><Brain className="h-4 w-4 text-accent" /> Faça uma previsão</h2>
            ...
            <div className="mt-5 space-y-5">
                ...
                <div className="relative mt-2">
                  ...
                  {suggestions.length > 0 && (
                    <div className="absolute z-10 mt-1 w-full rounded-md border border-border bg-popover shadow-elegant overflow-hidden">
                      {suggestions.map((s) => (
                        <button key={s} type="button" onClick={() => { setSkills([...skills, s]); setSkillInput(""); }} className="block w-full text-left px-3 py-2 text-sm hover:bg-secondary transition-colors">
                          {s}
                        </button>
                      ))}
                    </div>
                ...
                <div className="flex flex-wrap gap-1.5 mt-3">
                  {skills.map((s) => (
                    <Badge key={s} variant="secondary" className="gap-1 pr-1">
                      {s}
                      <button onClick={() => setSkills(skills.filter((x) => x !== s))} className="rounded hover:bg-background/50 p-0.5"><X className="h-3 w-3" /></button>
                    </Badge>
                  ))}
                </div>
              </div>
              ...
              <div className="rounded-lg border border-accent/30 bg-accent/5 p-5">
                <div className="text-xs uppercase tracking-widest text-accent">Salário estimado</div>
              ...
              <Button
                variant="outline"
                className="w-full"
                onClick={() => { setSkills([]); setSkillInput(""); }}
              >
                Limpar skills
              </Button>
            </div>
          ...
          <div className="space-y-8">
              ...
              <div className="h-80 mt-5">
                <ResponsiveContainer width="100%" height="100%">
                  <ScatterChart margin={{ top: 10, right: 20, bottom: 10, left: 10 }}>
                    ...
                    <XAxis
                      type="number" dataKey="actual" name="Real"
                      tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 11 }}
                      label={{ value: "Salário real (R$)", position: "insideBottom", offset: -5, fill: "hsl(var(--muted-foreground))", fontSize: 11 }}
                    />
                    <YAxis
                      type="number" dataKey="predicted" name="Previsto"
                      tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 11 }}
                      label={{ value: "Previsto (R$)", angle: -90, position: "insideLeft", fill: "hsl(var(--muted-foreground))", fontSize: 11 }}
                    />
                    <Tooltip
                    ...
                    <ReferenceLine
                      ...
                      stroke="hsl(var(--accent))" strokeDasharray="4 4"
                    />
                    <Scatter data={model.predictions} fill="hsl(var(--primary))" fillOpacity={0.7} />
                  </ScatterChart>
                </ResponsiveContainer>
              </div>
            </Card>
            ...
            <Card className="p-6 bg-card border-border/50">
                  ...
                  return (
                    <div key={r.name}>
                      <div className="flex items-center justify-between text-xs mb-1">
                        ...
                        <span className={`font-mono ${positive ? "text-accent" : "text-destructive"}`}>
              ...
              <p className="text-xs text-muted-foreground mt-5 leading-relaxed">
                <strong className="text-foreground">Como ler:</strong> com tudo o mais constante, possuir essa
                característica altera o salário previsto em ± o valor mostrado. O modelo inclui regularização L2
                (ridge λ=1e-6) para estabilidade numérica.
              </p>
            </Card>
            ...
            <Card className="p-6 bg-card/60 border-border/50">
              ...
              <ul className="mt-3 space-y-1.5 text-sm text-muted-foreground list-disc list-inside">
                <li><strong className="text-foreground">Variável alvo:</strong> média entre salário mínimo e máximo da vaga.</li>
                <li><strong className="text-foreground">Variáveis preditoras:</strong> senioridade (ordinal 0–3), nº total de skills, one-hot de área, one-hot de modelo de trabalho e indicadores binários das {MODEL_SKILLS.length} skills mais frequentes.</li>
                <li><strong className="text-foreground">Treino/Teste:</strong> divisão determinística 75% / 25% (a cada 4ª amostra vai para teste).</li>
                <li><strong className="text-foreground">Estimação:</strong> equações normais com solução fechada, calculadas em JavaScript puro (sem libs de ML).</li>
                <li><strong className="text-foreground">Métricas:</strong> R² (variância explicada), MAE (erro médio absoluto) e RMSE (raiz do erro quadrático médio).</li>
              </ul>
            </Card>
          </div>
        </div>
