  ...
  head: () => ({
    meta: [
      { title: "Recomendações de Vagas — CarreiraTech" },
      {
        name: "description",
        content:
          "Informe seu perfil e receba recomendações personalizadas de vagas compatíveis com suas habilidades e nível de experiência.",
      },
    ],
  }),
      ...
      <main className="flex-1 mx-auto w-full max-w-7xl px-6 py-12">
        <div className="mb-10 max-w-2xl">
          ...
          <p className="mt-3 text-muted-foreground">
            Conte sobre suas habilidades, nível e preferências. Calculamos um score de
            compatibilidade combinando skills, senioridade, área e modelo de trabalho.
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-[400px_1fr] gap-8">
          ...
          <Card className="p-6 bg-card border-border/50 h-fit lg:sticky lg:top-24">
            <h2 className="font-semibold flex items-center gap-2">
              <Sparkles className="h-4 w-4 text-accent" /> Seu perfil
            </h2>
            ...
            {submitted && recommendations.length === 0 && (
              <Card className="p-12 text-center bg-card border-border/50">
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  Tente adicionar mais skills ou ajustar os filtros.
                </p>
              </Card>
            )}
              ...
              <>
                <div className="mb-5 flex items-center justify-between">
                  <h2 className="font-semibold">
                    {recommendations.length} vagas recomendadas para você
                  </h2>
                  <span className="text-xs text-muted-foreground flex items-center gap-1">
                    <TrendingUp className="h-3 w-3" /> Ordenadas por compatibilidade
                  </span>
                </div>
                <div className="space-y-4">
                  {recommendations.map((r) => (
                    <Card
                      key={r.job.id}
                      className="p-6 bg-card border-border/50 hover:border-primary/40 transition-colors"
                    >
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="mt-1 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
                            <span className="flex items-center gap-1">
                              <Building2 className="h-3 w-3" /> {r.job.company}
                            </span>
                            <span className="flex items-center gap-1">
                              <MapPin className="h-3 w-3" /> {r.job.location}
                            </span>
                        ...
                        <div className="text-right shrink-0">
                          <div className="text-3xl font-display font-bold text-gradient leading-none">
                            {r.score}
                          </div>
                      ...
                      <div className="mt-4 flex items-baseline justify-between">
                        <span className="font-mono text-sm text-accent font-semibold">
                          R$ {r.job.salaryMin.toLocaleString("pt-BR")} –{" "}
                          {r.job.salaryMax.toLocaleString("pt-BR")}
                        </span>
                      </div>
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1">
                            ...
                            {r.matchedSkills.map((s) => (
                              <Badge
                                key={s}
                                className="bg-accent/15 text-accent border-accent/30 text-[10px] gap-1"
                              >
